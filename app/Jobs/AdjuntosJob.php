...
class AdjuntosJob implements ShouldQueue
{
    ...
    {
        $clientes = Cliente::whereNotNull('clientes.selfie')->select('id','selfie'); // Aseguramos que el cliente tenga una selfie
        ...
        $clientes->chunk(100, function ($clientes) {
            foreach ($clientes as $c) {
                Adjunto::firstOrCreate(
                    [
                        'cliente_id' => $c->id,
                        'tipo' => 'selfie' // Si quieres que el tipo también sea parte del criterio de unicidad
                    ],
                    [
                        'cliente_id' => $c->id,
                        'tipo' => 'selfie',
                        'nombre' => $c->selfie,
                        'url' => 'clientes/'.$c->selfie,
                        'path' => 'clientes'
                    ]
                );
            }
